...
public class MessageController {
    ...
    @PostMapping("/send")
    public MessageResponseTemplate sendMessage(@RequestBody MessageRequestTemplate messageRequestTemplate) {
        return messageService.sendMessage(messageRequestTemplate);
    }

    @GetMapping("/load/{userA}/{userB}")
    public List<MessageResponseTemplate> loadMessages(@PathVariable String userA, @PathVariable String userB) {
